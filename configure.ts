...
export async function configure(command: ConfigureCommand) {
  ...
  const options = await ask.script([
    ...
    {
      type: 'boolean',
      key: 'changeExceptionHandler',
      question: 'would you like to use dreamer as default exception handler?',
    },
    {
      type: 'string',
  ...
  if (options.changeExceptionHandler) {
    const handler = project.addSourceFileAtPath(command.app.exceptionsPath('handler.ts'))
    changeDefaultExceptionHandler(handler)
    await prettify([command.app.exceptionsPath('handler.ts')])
